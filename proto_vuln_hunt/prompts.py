...
class PromptBuilder:
    ...
    def audit(self, item: Dict[str, Any], lens_key: str, idx: int, schema) -> str:
        ...
        return (
            ...
            "- **潜在风险登记**:看到可疑但本次未坐实为漏洞的隐患/可加固点,放进 risk_notes(area/note/可选 file/severity_hint)——汇总进 RISKS.md 供人工复核,不要硬凑成 finding。\n"
    ...
    def verify(self, f: Dict[str, Any], lens: str, schema) -> str:
        return (
            ...
            "只有当你能 caller-trace **证伪可达性**、或**确证上游校验确实把输入夹紧**、或**确证根本不是 bug**时,才 is_real=false 并说明依据。\n"
